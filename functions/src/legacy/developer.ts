...
export const developerLogin = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
...
export const developerLogout = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).json({ error: "Method not allowed" });
            return;
...
export const createCompanyHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const getSystemLogsHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
...
export const getCompaniesHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
...
export const getCompanyUsersHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
        // @ts-ignore
...
export const updateUserPermissionsHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const getDashboardStatsHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
...
export const getCompanyFullDetailsHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
        // @ts-ignore
        const { companyId } = req.query;
...
export const updateCompanyPlanHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const toggleCompanyStatusHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const saveBlogPostHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const submitInquiryHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
...
export const getInquiriesHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        await verifyDeveloperSession(req);
...
export const getBlogPostsHttp = onRequest({ cors: true }, async (req, res) => {
...
export const getPublicBlogsHttp = onRequest({ cors: true }, async (req, res) => {
    // Set CORS headers explicitly
    res.set('Access-Control-Allow-Origin', '*');
...
export const getBlogPostBySlugHttp = onRequest({ cors: true }, async (req, res) => {
    // Set CORS headers explicitly
    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'GET, OPTIONS');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
...
export const saveMarketingContentHttp = onRequest({ cors: true }, async (req, res) => {
    try {
        if (req.method !== "POST") {
            res.status(405).end();
            return;
        ...
        await db.collection("metadata").doc("landingContent").set({
            ...content,
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
        });
